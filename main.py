...
from report_generators.tts_adoption import TtsAdoption
...
def generate_tts_adoption_report():
    df = pd.read_csv(config['primary_snapshot_url'], low_memory=False)
    tts_adoption_report = TtsAdoption(df)
    report = tts_adoption_report.generate_report()
    report.to_csv(config['tts_adoption_report_location'], index=False)
...
if __name__ == '__main__':
    ...
    valid_commands = {
        ...
        'generate-tts-adoption-report': generate_tts_adoption_report
    }
